...
export default function AdminDashboard() {
  ...
  const handleFocus = useCallback(() => {
    ...
    // Only refresh if data has been loaded and we're not already loading or processing buttons
  ...
  const handleVisibilityChange = useCallback(() => {
    if (document.visibilityState === 'visible' && dataLoadedRef.current) {
      console.log('Admin Dashboard: Page became visible, refreshing data')
      // Only refresh if we're not already loading
      if (!invitationsLoading && !activityLoading && !buttonProcessing) {
        handleRefreshAll()
      }
    }
  }, [invitationsLoading, activityLoading, buttonProcessing])
  ...
  const handleRouteChange = useCallback((url: string) => {
    ...
    // Only refresh if we're on the admin dashboard and not already loading
    if (url.includes('/admin/dashboard') && dataLoadedRef.current && !invitationsLoading && !activityLoading && !buttonProcessing) {
        ...
        if (validatedSession && validatedSession.userData.isAdmin) {
          ...
          if (isRefreshInvitations) {
            ...
            setTimeout(async () => {
              ...
              // Remove the refresh parameter from URL using Next.js router
  ...
  // Event-driven refresh for admin dashboard - DISABLED to prevent loops
  // useEffect(() => {
  //   if (!isAdmin || invitationsLoading) return

  //   console.log('Admin Dashboard: Setting up event-driven refresh for admin dashboard')
      
  //   // Check for changes every 3 seconds when admin dashboard is visible
  ...
  //       console.log('Admin Dashboard: Event-driven refresh - checking for changes')
  //       await handleRefreshAll()
  //     }
  //   }, 3000)

  //   return () => {
  //     console.log('Admin Dashboard: Clearing event-driven refresh intervals')
  //     clearInterval(interval)
  //   }
  // }, [isAdmin, invitationsLoading, buttonProcessing])
  ...
  const loadCompaniesCount = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        console.log('Admin Dashboard: Companies count loaded:', data.totalCount)
        setCompaniesCount(data.totalCount || 0)
      } else {
  ...
  const handleApprove = useCallback(async (invitationId: number) => {
    ...
    try {
      ...
      await loadInvitations()
      console.log('Admin Dashboard: Reloading recent activity after approval...')
      await loadRecentActivity()
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors disabled:opacity-50 flex items-center gap-2"
