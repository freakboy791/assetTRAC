...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  console.log('=== SEND INVITE EMAIL API CALLED ===')
  ...
  try {
    ...
    console.log('Send invite API: About to call Edge Function...')
    console.log('Send invite API: Supabase URL:', supabaseUrl)
    console.log('Send invite API: Edge Function URL:', `${supabaseUrl}/functions/v1/send-invite-email`)
    console.log('Send invite API: Service key present:', !!process.env.SUPABASE_SERVICE_ROLE_KEY)
    ...
    try {
      ...
      console.log('Send invite API: Edge Function response status:', emailResponse.status)
      console.log('Send invite API: Edge Function response headers:', Object.fromEntries(emailResponse.headers.entries()))
      
      const emailResult = await emailResponse.json()
      console.log('Send invite API: Edge Function response body:', emailResult)
      
      if (!emailResponse.ok) {
        console.error('Send invite API: Email function error:', emailResult)
        // Don't fail the whole process if email fails, just log it
        console.log('Send invite API: Email sending failed, but invitation record was created')
      } else {
        console.log('Send invite API: Email sent successfully:', emailResult)
      }
    } catch (emailError) {
