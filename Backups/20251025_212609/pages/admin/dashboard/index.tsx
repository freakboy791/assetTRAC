...
export default function AdminDashboard() {
  ...
  const handleFocus = useCallback(() => {
    console.log('Admin Dashboard: Focus event triggered, loading state:', { invitationsLoading, activityLoading, buttonProcessing, dataLoaded: dataLoadedRef.current })
    ...
    if (!dataLoadedRef.current && !invitationsLoading && !activityLoading && !buttonProcessing) {
      console.log('Admin Dashboard: Refreshing data on focus (first visit)')
      handleRefreshAll()
    } else {
      console.log('Admin Dashboard: Skipping focus refresh - data already loaded or loading')
    }
  }, [invitationsLoading, activityLoading, buttonProcessing])

  const handleVisibilityChange = useCallback(() => {
    if (document.visibilityState === 'visible') {
      console.log('Admin Dashboard: Page became visible, loading state:', { invitationsLoading, activityLoading, buttonProcessing, dataLoaded: dataLoadedRef.current })
      // Only refresh if data hasn't been loaded yet (first visit)
      if (!dataLoadedRef.current && !invitationsLoading && !activityLoading && !buttonProcessing) {
        console.log('Admin Dashboard: Refreshing data on visibility change (first visit)')
        handleRefreshAll()
      } else {
        console.log('Admin Dashboard: Skipping visibility refresh - data already loaded or loading')
      }
    }
  }, [invitationsLoading, activityLoading, buttonProcessing])

  const handleRouteChange = useCallback((url: string) => {
    console.log('Admin Dashboard: Route changed to:', url)
    ...
    if (url.includes('/admin/dashboard') && !invitationsLoading && !activityLoading && !buttonProcessing) {
      console.log('Admin Dashboard: Refreshing data after route change')
  ...
  const handleAdminAction = useCallback((action: string) => {
    console.log('Admin Dashboard: Admin action detected:', action)
    if (dataLoadedRef.current && !invitationsLoading && !activityLoading && !buttonProcessing) {
      console.log('Admin Dashboard: Refreshing data after admin action:', action)
      handleRefreshAll()
    }
  }, [invitationsLoading, activityLoading, buttonProcessing])

  const handleCustomRefresh = useCallback((event: CustomEvent) => {
    console.log('Admin Dashboard: Custom refresh event received:', event.detail)
  ...
  const getDisplayName = () => {
    console.log('Admin Dashboard: getDisplayName called with user:', user)
    console.log('Admin Dashboard: user.first_name:', user?.first_name)
    console.log('Admin Dashboard: user.last_name:', user?.last_name)
    console.log('Admin Dashboard: user.email:', user?.email)
    
    if (user?.first_name && user?.last_name) {
      const name = `${user.first_name} ${user.last_name}`
      console.log('Admin Dashboard: Returning full name:', name)
      return name
    } else if (user?.first_name) {
      console.log('Admin Dashboard: Returning first name only:', user.first_name)
      return user.first_name
    } else if (user?.last_name) {
      console.log('Admin Dashboard: Returning last name only:', user.last_name)
      return user.last_name
    } else if (user?.user_metadata?.first_name) {
      console.log('Admin Dashboard: Returning metadata first name:', user.user_metadata.first_name)
      return user.user_metadata.first_name
    } else {
      console.log('Admin Dashboard: Returning email as fallback:', user?.email || 'User')
        ...
        if (validatedSession && validatedSession.userData.isAdmin) {
          ...
          try {
            ...
            if (response.ok) {
              ...
              console.log('Admin Dashboard: Fresh user data from API:', data.user)
              console.log('Admin Dashboard: User first_name:', data.user.first_name)
              console.log('Admin Dashboard: User last_name:', data.user.last_name)
              console.log('Admin Dashboard: User email:', data.user.email)
              setUser(data.user) // This now includes profile data
            } else {
              console.error('Failed to fetch user data, using session data')
              console.log('Admin Dashboard: Using session user data:', validatedSession.user)
          ...
          console.log('Admin Dashboard: Loading data for admin dashboard')
  ...
  useEffect(() => {
    ...
    console.log('Admin Dashboard: Setting up polling refresh for admin dashboard')
    
    // Add a delay before starting the polling to avoid interfering with natural page loads
    const initialDelay = setTimeout(() => {
      console.log('Admin Dashboard: Starting polling refresh after initial delay')
      ...
      const interval = setInterval(async () => {
        ...
        if (document.visibilityState === 'visible' && document.hasFocus() && !buttonProcessing) {
          console.log('Admin Dashboard: Polling refresh - checking for changes')
    ...
    return () => {
      console.log('Admin Dashboard: Clearing polling refresh intervals')
  ...
  const loadInvitations = async () => {
    ...
    try {
        ...
        if (data) {
          data.forEach((inv: any, index: number) => {
            console.log(`Admin Dashboard: Invitation ${index}:`, {
              ...
              shouldShowButtons: inv.status === 'email_confirmed' && !inv.admin_approved_at
            })
          })
        }
      ...
      console.log('Admin Dashboard: Filtered invitations:', filteredInvitations.map(inv => ({ id: inv.id, email: inv.invited_email, status: inv.status })))
  ...
  const loadCompletedUsers = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        console.log('Admin Dashboard: Completed users loaded:', data.users?.length || 0, 'users')
  ...
  const loadCompaniesCount = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        console.log('Admin Dashboard: Companies count loaded:', data.companiesCount)
  ...
  const handleApprove = useCallback(async (invitationId: number) => {
    ...
    console.log('Admin Dashboard: handleApprove - Starting approval process...')
    setButtonProcessing(true) // Prevent focus events from interfering
    setInvitationsLoading(true)
    try {
      // Get the current tab-specific session token with enhanced validation
      console.log('Admin Dashboard: handleApprove - Getting tab ID...')
      const tabId = getTabId()
      console.log('Admin Dashboard: handleApprove - Tab ID:', tabId)
      
      console.log('Admin Dashboard: handleApprove - Validating session...')
      const { session: validatedSession, error: sessionError } = await validateAndRefreshSession(tabId)
      
      console.log('Admin Dashboard: handleApprove - Session validation result:', {
        hasSession: !!validatedSession,
        hasAccessToken: !!validatedSession?.accessToken,
        error: sessionError?.message
      })
      
      if (sessionError || !validatedSession || !validatedSession.accessToken) {
        console.error('Admin Dashboard: No valid session found for approving user:', sessionError?.message)
        console.log('Admin Dashboard: handleApprove - Redirecting to login due to session error')
      ...
      console.log('Admin Dashboard: handleApprove - Making API call to /api/admin/approve-user...')
      ...
      console.log('Admin Dashboard: handleApprove - API response received:', {
      ...
      console.log('Admin Dashboard: handleApprove - Approve request successful!')
      
      // Trigger admin refresh for user approval
      console.log('Admin Dashboard: handleApprove - Triggering user refresh...')
      triggerUserRefresh.approved()
      
      // Reload invitations, recent activity, and completed users
      console.log('Admin Dashboard: handleApprove - Reloading invitations after approval...')
      await loadInvitations()
      console.log('Admin Dashboard: handleApprove - Reloading recent activity after approval...')
      await loadRecentActivity()
      console.log('Admin Dashboard: handleApprove - Reloading completed users after approval...')
      await loadCompletedUsers()
      
      console.log('Admin Dashboard: handleApprove - Approval process completed successfully!')
    } catch (error) {
      ...
      console.log('Admin Dashboard: handleApprove - Cleaning up loading states...')
      setInvitationsLoading(false)
      setButtonProcessing(false) // Re-enable focus events
      console.log('Admin Dashboard: handleApprove - Cleanup completed')
    }
  }, [])
  ...
  const handleReject = useCallback(async (invitationId: number) => {
    ...
    try {
      ...
      // Reload invitations, recent activity, and completed users
      console.log('Admin Dashboard: Reloading invitations after rejection...')
      await loadInvitations()
      console.log('Admin Dashboard: Reloading recent activity after rejection...')
      await loadRecentActivity()
      console.log('Admin Dashboard: Reloading completed users after rejection...')
      await loadCompletedUsers()
    } catch (error) {
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-col items-end space-y-2">
                          ...
                          {(() => {
                            ...
                            console.log(`Admin Dashboard: Button logic for invitation ${invitation.id}:`, {
                            ...
                            <div className="flex items-center space-x-2">
                              <button
                                onClick={(e) => {
                                  console.log('Admin Dashboard: APPROVE BUTTON CLICKED!', invitation.id);
                                  e.preventDefault();
                                  e.stopPropagation();
                                  handleApprove(invitation.id);
                                }}
                                onMouseDown={() => console.log('Admin Dashboard: Approve button mouse down')}
                                onMouseUp={() => console.log('Admin Dashboard: Approve button mouse up')}
                                onMouseEnter={() => console.log('Admin Dashboard: Approve button mouse enter')}
                                onMouseLeave={() => console.log('Admin Dashboard: Approve button mouse leave')}
                              ...
                              <button
                                onClick={(e) => {
                                  console.log('Admin Dashboard: DENY BUTTON CLICKED!', invitation.id);
                                  alert('Deny button clicked!');
                                  e.preventDefault();
                                  e.stopPropagation();
                                  handleReject(invitation.id);
                                }}
                                onMouseDown={() => console.log('Admin Dashboard: Deny button mouse down')}
                                onMouseUp={() => console.log('Admin Dashboard: Deny button mouse up')}
                                onMouseEnter={() => console.log('Admin Dashboard: Deny button mouse enter')}
                                onMouseLeave={() => console.log('Admin Dashboard: Deny button mouse leave')}
