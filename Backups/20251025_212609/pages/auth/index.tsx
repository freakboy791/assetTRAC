  ...
  const handleLogIn = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        } else if (result.message.includes('Account not activated') || result.message.includes('waiting for admin approval')) {
          setErrorType('admin_approval_pending')
        ...
        try {
          ...
          if (updateResponse.ok) {
            console.log('Auth: Last login timestamp updated successfully')
          } else {
            console.error('Auth: Failed to update last login timestamp')
          }
        } catch (updateError) {
          console.error('Auth: Error updating last login timestamp:', updateError)
        }
