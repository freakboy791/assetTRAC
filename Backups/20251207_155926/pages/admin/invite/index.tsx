...
export default function AdminInvitePage() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      try {
        ...
        // Check if user has permission to send invitations using fresh data or fallback to session data
        const checkData = freshUserData || userData
        const rolesToCheck = freshUserData?.roles || userData?.roles || []
        const isAdminCheck = checkData.isAdmin || false
        const isOwnerCheck = rolesToCheck.includes('owner')
        const isManagerCheck = rolesToCheck.some((role: string) => role.startsWith('manager'))
        
        const canSendInvitations = isAdminCheck || isOwnerCheck || isManagerCheck
        
        if (!canSendInvitations) {
          // Redirect immediately without showing error message
          window.location.href = isAdminCheck ? '/admin/dashboard' : '/dashboard'
          return
        }
        
        // Only set loading to false if permissions are correct
        setLoading(false)
      } catch (error) {
  ...
  // Only check after loading is complete to avoid showing error message during initial load
  const canSendInvitations = !loading && (isAdmin || userRoles.includes('owner') || userRoles.some(role => role.startsWith('manager')))
  
  if (!loading && !canSendInvitations) {
    return (
