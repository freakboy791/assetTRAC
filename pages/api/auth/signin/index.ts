  ...
  try {
    ...
    if (!inviteError && invitation && !invitation.completed_at) {
      ...
      if (invitation.status === 'admin_approved') {
        ...
        try {
          // Get user's profile to check last_login_at
        ...
        // Check if this is actually a first login before logging activity for email_confirmed users
        try {
          // Get user's profile to check last_login_at
          const { data: userProfile } = await supabaseAdmin()
            .from('profiles')
          ...
          if (isFirstLogin) {
            ...
            console.log('Signin API: Not a first login for email_confirmed user, skipping activity logging')
          }
        } catch (logError) {
          console.error('Signin API: Error checking/logging first login activity for email_confirmed user:', logError)
        }
      }
    }
