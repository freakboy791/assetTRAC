      ...
      try {
        ...
        if (inviteData.invitation.status === 'pending') {
          ...
          if (error) {
            ...
            // Successful login - update last login timestamp and set session storage

            
            // Update last login timestamp and log first login activity
            try {
              ...
              if (updateResponse.ok) {
                ...
                console.error('Index: Failed to update last login timestamp')
              }
              
              // Check if this is actually a first login before logging activity
              try {
                const profileResponse = await fetch('/api/auth/getUser', {
                  method: 'GET',
                  headers: {
                    'Authorization': `Bearer ${data.session.access_token}`,
                    'Content-Type': 'application/json'
                ...
                console.error('Index: Error checking/logging first login activity:', activityError)
              }
              
            } catch (updateError) {
              console.error('Index: Error updating last login timestamp:', updateError)
            }
        ...
        } else if (inviteData.invitation.status === 'expired') {
          // User account has been denied by admin
          setMessage('Your account has been rejected by the administrator. Please contact your administrator for assistance.')
          setLoading(false)
          return
        } else {
